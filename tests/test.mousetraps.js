...
var Mousetraps = Mousetraps || require('../Mousetraps');
var KeyEvent = KeyEvent || require('./libs/key-event');



// Reset Mousetraps after each test
afterEach(function () {
  Mousetraps.reset();
});

describe('Mousetraps.bind', function () {
  describe('basic', function () {
    it('z key fires when pressing z', function () {
      var spy = sinon.spy();

      Mousetraps.bind('z', spy);
    ...
    it('Adding a new key callback does not overwrite any existing callbacks for that key', function () {
      var spy1 = sinon.spy();
      var spy2 = sinon.spy();
      Mousetraps.bind('x', spy1);
      Mousetraps.bind('x', spy2);

      KeyEvent.simulate('X'.charCodeAt(0), 88);

      expect(spy1.callCount).to.equal(1, 'original callback should fire');
      expect(spy2.callCount).to.equal(1, 'new callback should fire');
    });

    it('binding an array of keys', function () {
      var spy = sinon.spy();
      Mousetraps.bind(['a', 'b', 'c'], spy);
    ...
    it('return false should prevent default and stop propagation', function () {
      ...
      spy = sinon.spy();
      Mousetraps.bind('command+d', spy);
      KeyEvent.simulate('D'.charCodeAt(0), 68, ['meta']);
    ...
    it('keypress after keydown', function () {
      var spy = sinon.spy();
      var spy2 = sinon.spy();
      Mousetraps.bind('a', spy, "keydown");
      Mousetraps.bind('a', spy2, "keypress");

      KeyEvent.simulate('a'.charCodeAt(0), 65);
      expect(spy.callCount).to.equal(1, 'keydown bind should fire');
      expect(spy2.callCount).to.equal(1, 'keypress bind should fire');
    });
  });
  ...
  describe('combos with modifiers', function () {
    ...
    it('binding key combos with multiple modifiers', function () {
      var spy = sinon.spy();
      Mousetraps.bind('command+shift+o', spy);
      KeyEvent.simulate('O'.charCodeAt(0), 79, ['meta']);
  ...
  describe('sequences', function () {
    ...
    it('binding sequences starting with modifier keys', function () {
      var spy = sinon.spy();
      Mousetraps.bind('option enter', spy);
      KeyEvent.simulate(0, 18, ['alt']);
      KeyEvent.simulate(0, 13);
      expect(spy.callCount).to.equal(1, 'callback should fire');

      spy = sinon.spy();
      Mousetraps.bind('command enter', spy);
      KeyEvent.simulate(0, 91, ['meta']);
      KeyEvent.simulate(0, 13);
      expect(spy.callCount).to.equal(1, 'callback should fire');

      spy = sinon.spy();
      Mousetraps.bind('escape enter', spy);
    ...
    it('key within sequence should not fire', function () {
      var spy1 = sinon.spy();
      var spy2 = sinon.spy();
      Mousetraps.bind('a', spy1);
      Mousetraps.bind('c a t', spy2);
    ...
    it('keyup at end of sequence should not fire', function () {
      ...
      Mousetraps.bind('t', spy1, 'keyup');
      Mousetraps.bind('b a t', spy2);
    ...
    it('keyup sequences should work', function () {
      var spy = sinon.spy();
      Mousetraps.bind('b a t', spy, 'keyup');
    ...
    it('modifiers and sequences play nicely', function () {
      ...
      Mousetraps.bind('ctrl a', spy1);
      Mousetraps.bind('ctrl+b', spy2);
    ...
    it('sequences that start the same work', function () {
      ...
      Mousetraps.bind('g g l', spy2);
      Mousetraps.bind('g g o', spy1);
    ...
    it('sequences should not fire subsequences', function () {
      ...
      Mousetraps.bind('a b c', spy1);
      Mousetraps.bind('b c', spy2);
      ...
      spy1.resetHistory();
      spy2.resetHistory();
      Mousetraps.bind('option b', spy1);
      Mousetraps.bind('a option b', spy2);
    ...
    it('binding same sequence should not override previous', function () {
      var spy1 = sinon.spy();
      var spy2 = sinon.spy();
      Mousetraps.bind('a b c', spy1);
      Mousetraps.bind('a b c', spy2);

      KeyEvent.simulate('a'.charCodeAt(0), 65);
      KeyEvent.simulate('b'.charCodeAt(0), 66);
      KeyEvent.simulate('c'.charCodeAt(0), 67);

      expect(spy1.callCount).to.equal(1, 'first callback should fire');
      expect(spy2.callCount).to.equal(1, 'second callback should fire');
    });

    it('broken sequences', function () {
      var spy = sinon.spy();
      Mousetraps.bind('h a t', spy);
    ...
    it('sequences containing combos should work', function () {
      ...
      Mousetraps.unbind('a ctrl+b', spy);

      spy = sinon.spy();
      Mousetraps.bind('ctrl+b a', spy);
    ...
    it('sequences starting with spacebar should work', function () {
      var spy = sinon.spy();
      Mousetraps.bind('a space b c', spy);
    ...
    it('konami code', function () {
      var spy = sinon.spy();
      Mousetraps.bind('up up down down left right left right b a enter', spy);
...
describe('Mousetraps.unbind', function () {
...
describe('wrapping a specific element', function () {
  ...
  it('z key does not fire when pressing z outside the target element', function () {
    ...
    Mousetraps(textarea).bind('z', spy);
  ...
  it('should work when constructing a new Mousetraps object', function () {
    var spy = sinon.spy();

    var mousetraps = new Mousetraps(form);
    mousetraps.bind('a', spy);
  ...
  it('should allow you to create an empty Mousetraps constructor', function () {
    var spy = sinon.spy();

    var mousetraps = new Mousetraps();
    mousetraps.bind('a', spy);
...
describe('Mouestrap.addKeycodes', function () {
  it('should properly recognize non-default mapping', function () {
    ...
    Mousetraps.addKeycodes({
      144: 'num',
    });

    Mousetraps.bind('num', spy);
